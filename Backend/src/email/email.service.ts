...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.locationVerifierEmail =
      this.configService.get<string>('LOCATION_VERIFIER_EMAIL') ||
      'formulario.eventos@unicesusc.edu.br';
    this.frontendUrl =
  ...
  async sendLocationSupportTeamsNotification(
    ...
    const teamNames = teams.map((t) => t.name).join(', ');
    const formUrl = `${this.frontendUrl}/form/${location.id}`;
